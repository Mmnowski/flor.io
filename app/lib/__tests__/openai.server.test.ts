...
describe('openai.server', () => {
  describe('generateCareInstructions', () => {
    ...
    it('handles scientific names case-insensitively', async () => {
      const result1 = await generateCareInstructionsInstant('monstera deliciosa');
      const result2 = await generateCareInstructionsInstant('MONSTERA DELICIOSA');
      const result3 = await generateCareInstructionsInstant('Monstera deliciosa');

      // All should match the same plant
      expect(result1.wateringFrequencyDays).toBe(result3.wateringFrequencyDays);
      expect(result2.wateringFrequencyDays).toBe(result3.wateringFrequencyDays);
    });
  ...
  describe('generateCareInstructionsInstant', () => {
    it('returns a valid care instructions object immediately', async () => {
      ...
      const result = await generateCareInstructionsInstant('Monstera deliciosa');
  ...
  describe('Response consistency', () => {
    it('multiple calls return consistent data for same plant', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const result = await generateCareInstructionsInstant('Ficus elastica');
        results.push(result);
      }
